...
- 인식 범위 표시: 경계선 주변의 명도 탐색 반경을 px 단위로 설정하고 이미지 위에 반투명 영역으로 확인
...
6. **인식 설정**에서 명도 탐색 반경을 조절합니다. **범위 표시**가 켜져 있으면 이미지 위에 탐색 영역이 표시됩니다.
7. **인식**을 누르면 경계선이 설정된 탐색 범위 안에서 명도 변화 최대 위치로 스냅됩니다.
8. 필요하면 가이드 방향/간격을 정하고 **그리기**를 누릅니다.
9. **각도 계산**으로 기준선 대비 각도와 가이드 교점 각도를 표시합니다.
10. 결과는 **CSV 내보내기** 또는 **주석 PNG 내보내기**로 저장합니다.

## 경계 인식 방식

사용자가 그은 경계선을 중심으로 선분에 수직인 방향의 픽셀 밝기 프로파일을 만듭니다. 탐색 반경은 **인식 설정** 또는 툴바의 **탐색 px** 값으로 조절합니다. 예를 들어 20 px이면 경계선 양쪽 20 px, 총 40 px 폭 안에서만 명도 변화를 찾습니다. 각 오프셋 위치에서 선분을 따라 평균 명도를 샘플링하고, 이 1차원 프로파일의 기울기 절댓값이 최대인 오프셋을 실제 경계로 판단합니다. 선택한 선분만 인식하려면 선분을 먼저 선택한 뒤 **인식**을 누르면 됩니다. 아무 선분도 선택하지 않으면 모든 경계선이 인식됩니다.
