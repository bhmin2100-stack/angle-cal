...
- 기준선 정렬: 이미지마다 기준선 하나를 두고, 수평 기준선/평행 기준선 토글 후 이미지 전체를 기준축에 맞춰 회전
...
- 경계인식 범위 표시: 경계선 주변의 인식 반경을 px 단위로 설정하고 이미지 위에 반투명 초록색 영역으로 확인
...
6. 기준 종류를 **수평 기준선** 또는 **평행 기준선**으로 고른 뒤 **이미지 맞춤**을 누릅니다. 기준선 도구가 선택된 상태에서 토글을 바꾸고 다시 **이미지 맞춤**을 누르면 같은 기준선이 새 축 기준으로 정렬됩니다.
...
9. **인식 설정**에서 경계인식 범위와 세그먼트 크기를 조절합니다. 값을 바꾸면 이미지 우상단에 샘플이 표시되고, **범위 표시**가 켜져 있으면 이미지 위에 반투명 초록색 경계인식 영역과 px 폭이 표시됩니다.
...
13. `Esc`를 누르면 선택 도구로 돌아갑니다. 드래그 박스로 여러 개체를 선택해 옮기거나 `Delete`로 삭제할 수 있습니다.
14. 선택한 개체는 **크기 조절** 도구에서 드래그해 확대·축소할 수 있습니다. 각도 숫자와 호는 하나의 묶음으로 같이 선택됩니다.
15. 오른쪽 아래 **표시** 레전드에서 묶음 단위로 보이기/숨기기를 전환할 수 있습니다.
16. 결과는 **CSV 내보내기** 또는 **주석 PNG 내보내기**로 저장합니다.
