...
7. **경계선** 버튼을 누르고 측정할 구조 경계를 대략 따라 그립니다.
8. **인식 설정**에서 명도 탐색 반경과 곡선 민감도를 조절합니다. **범위 표시**가 켜져 있으면 이미지 위에 반투명 초록색 탐색 영역과 px 폭이 표시됩니다.
9. **인식**을 누르면 경계선이 설정된 탐색 범위 안에서 명도 변화 최대 위치를 따라 곡선 경계로 변환됩니다.
10. 필요하면 가이드 방향/간격을 정하고 **그리기**를 누릅니다.
11. **각도 계산**으로 기준선 대비 각도와 가이드 교점 각도를 표시합니다.
12. `Esc`를 누르면 선택 도구로 돌아갑니다. 드래그 박스로 여러 개체를 선택해 옮기거나 `Delete`로 삭제할 수 있습니다.
13. 선택한 개체는 **크기 조절** 도구에서 드래그해 확대·축소할 수 있습니다. 각도 숫자와 호는 하나의 묶음으로 같이 선택됩니다.
14. 오른쪽 아래 **표시** 레전드에서 묶음 단위로 보이기/숨기기를 전환할 수 있습니다.
15. 결과는 **CSV 내보내기** 또는 **주석 PNG 내보내기**로 저장합니다.
