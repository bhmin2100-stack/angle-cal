...
def test_ctrl_tab_shows_shortcut_overlay_only_while_held():
    ...
    try:
        ...
        assert "Ctrl+Tab" in text.toPlainText()
        assert "Esc" in text.toPlainText()
        assert "Enter" in text.toPlainText()
        assert "스케일/가이드 그리기 Shift" in text.toPlainText()
        assert "Q + 드래그" in text.toPlainText()
        assert "E + 드래그" in text.toPlainText()
        assert "R 누르고 클릭/드래그" in text.toPlainText()
        assert "Ctrl + 클릭(세그먼트)" in text.toPlainText()
        assert "휠: 경계인식범위 조절" in text.toPlainText()
        assert "Ctrl+C" in text.toPlainText()
        assert "Ctrl+Shift+C" in text.toPlainText()
        assert "Ctrl+Shift+V" in text.toPlainText()
        assert "Ctrl+G" in text.toPlainText()
        assert "Ctrl+S" in text.toPlainText()
        assert "PageUp/PageDown" in text.toPlainText()
        assert "썸네일 Shift" in text.toPlainText()
        assert "1~9" in text.toPlainText()
        assert "드래그=좌측" not in text.toPlainText()
